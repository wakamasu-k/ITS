...
class TULIP(nn.Module):
    ...
    def build_layers_v2(self):
        ...
        for i in range(self.num_layers):
            layer = BasicBlockV2(
                index=i,
                input_resolution=(self.patch_embed.grid_size[0] // (2**i),
                              self.patch_embed.grid_size[1] // (2**i)),
    ...
    def build_layers_up_v2(self):
        ...
        for i in range(self.num_layers - 1):
            layer = BasicBlockUpV2(
                index=i,
                input_resolution=(self.patch_embed.grid_size[0] // (2**(self.num_layers-2-i)),
                              self.patch_embed.grid_size[1] // (2**(self.num_layers-2-i))),                depths=self.depths,
                embed_dim=self.embed_dim,
