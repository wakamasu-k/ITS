...
def npy_loader(path: str) -> np.ndarray:
    with open(path, "rb") as f:
        ...
        # range_map = range_intensity_map[..., 0]  ← この行を消すか # をつける
    return range_intensity_map.astype(np.float32) # ← 全体を返すように変更！    
...
class ScaleChannelZero(object):
    """0番目のチャンネル（距離）だけをスケールし、他（反射強度）は維持する"""
    def __init__(self, scale):
        self.scale = scale

    def __call__(self, tensor):
        tensor_clone = tensor.clone()
        tensor_clone[0, ...] = tensor_clone[0, ...] * self.scale
        return tensor_clone
...
def build_kitti_upsampling_dataset(is_train, args):
    ...
    t_low_res = [transforms.ToTensor(), ScaleChannelZero(1/80)]
    t_high_res = [transforms.ToTensor(), ScaleChannelZero(1/80)]
