...
def _embed_paths(
    ...
    with torch.no_grad():
        for x, idx in dl:
            ...
            elapsed_time = t_forward_end - t_forward_start
            batch_n = int(x.shape[0])
            per_image = elapsed_time / max(batch_n, 1)

            print(f"{elapsed_time=:.3f}", flush=True)
            print(
                f"[TIMING][retrieval] kind={kind} "
                f"batch_size={batch_n} "
                f"batch_forward_sec={elapsed_time:.6f} "
                f"per_image_sec={per_image:.6f}",
                flush=True,
            )
...
def _load_model(args):
    ...
    class ResNetVLAD(nn.Module):
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            ...
            print(
                f"[TIMING][netvlad] "
                f"kind={getattr(self, '_timing_kind', 'unknown')} "
                f"netvlad_sec={elapsed_time:.6f}",
                flush=True,
            )
            e = self.proj(v)
