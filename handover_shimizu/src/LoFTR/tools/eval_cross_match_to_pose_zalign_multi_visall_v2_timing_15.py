...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--timing-runs", type=int, default=15, help="Stop after this many [TIMING][dense] records are printed. 0 disables.")
...
def _apply_zalign(per_pair_df: pd.DataFrame, mode: str) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    timing_count += 1
    if args.timing_runs > 0 and timing_count >= args.timing_runs:
        print(f"[INFO] reached timing_runs={args.timing_runs}")
        raise SystemExit(0)
    return d, info
...
def main() -> None:
    ...
    for row in pbar:
        ...
        try:
            ...
            t_pnp_end = time.perf_counter()
            t_dense_end = time.perf_counter()

            loftr_time = t_loftr_end - t_loftr_start
            pnp_time = t_pnp_end - t_pnp_start
            dense_total_time = t_dense_end - t_dense_start

            print(
                f"[TIMING][dense] "
                f"pair_id={rdict.get('pair_id', '')} "
                f"loftr_sec={loftr_time:.6f} "
                f"pnp_sec={pnp_time:.6f} "
                f"total_sec={dense_total_time:.6f} "
                f"n_match={int(mk0.shape[0])} "
                f"n_3d={pose.n_3d} "
                f"n_inlier={pose.n_inlier} "
                f"pnp_success={pose.success}",
                flush=True,
            )
