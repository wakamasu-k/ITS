...
# -*- coding: utf-8 -*-
# 使い方:
#   python render_anchor_intensity_txt_se3_fix.py --help
...
def vehicle_to_cam_from(calib) -> np.ndarray:
    """Waymo の calibration に格納された 4x4 外部変換行列を返す。

    重要:
      元コードベースの規約では `calib.extrinsic.transform` を
      **vehicle<-camera**（camera->vehicle）として扱う。
      多くの投影式ではその逆行列 **camera<-vehicle** が必要になるため、
      呼び出し側では通常 `inv(T_vc)` を使う。

    この補助関数は主に、SE(3) 仮想アンカーレンダラが
    `vehicle_to_cam_from()` を呼ぶために用意している。
    """
    return mat4_list_to_np(calib.extrinsic.transform)



def world_to_cam_from_T_wv(T_wv: np.ndarray, calib) -> np.ndarray:
    """与えられた ego pose T_wv（vehicle->world）から World->Camera 変換を作る。

    これは仮想アンカーで使う厳密な SE(3) 版で、
      T_wc = T_cv * T_vw
    と表せる。ここで:
      T_wv : world <- vehicle  （vehicle->world）
      T_vw : vehicle <- world  （world->vehicle）= inv(T_wv)
      T_vc : vehicle <- camera （camera->vehicle）calib.extrinsic 由来
      T_cv : camera <- vehicle （vehicle->camera）= inv(T_vc)
    """
    T_vc = vehicle_to_cam_from(calib)  # vehicle <- camera
...
def _process_one_tfrecord(tfrecord_path: Path, args: argparse.Namespace, strict: bool) -> str:
    ...
    for sm in tqdm(submaps, desc="render anchors", unit="submap"):
        ...
        anchor_pose = sm.get('anchor_pose_xyyaw', None) if isinstance(sm, dict) else None
        if isinstance(anchor_pose, dict):
            ax = float(anchor_pose.get('x', 0.0))
            ay = float(anchor_pose.get('y', 0.0))
        elif isinstance(anchor_pose, (list, tuple)) and len(anchor_pose) >= 2:
            # 旧形式 / 代替形式 [x, y, yaw_deg] も許容する
            ax = float(anchor_pose[0])
            ay = float(anchor_pose[1])
        else:
            # 代替として、SE(3) があればそこから中心を求め、なければ実フレーム姿勢から求める
            if isinstance(sm, dict) and sm.get('anchor_pose_T_wv', None) is not None:
                T_tmp = mat4_list_to_np(sm['anchor_pose_T_wv'])
            else:
                T_tmp = mat4_list_to_np(frame.pose.transform)
            ax = float(T_tmp[0, 3])
            ay = float(T_tmp[1, 3])
        ...
        # 厳密な SE(3) 運用: submaps.json にアンカー用の明示的な ego pose があればそれを使う
        # （make_submaps_distance_virtual_startend_from_labels_se3.py が生成する仮想アンカーは、4x4 行列を 'anchor_pose_T_wv' に保存する。）
        if isinstance(sm, dict) and sm.get('anchor_pose_T_wv', None) is not None:
            T_wv_anchor = mat4_list_to_np(sm['anchor_pose_T_wv'])
        elif isinstance(sm, dict) and sm.get('T_wv_rowmajor', None) is not None:
            # 行優先 4x4 を直接保存している場合の代替キー名
            T_wv_anchor = mat4_list_to_np(sm['T_wv_rowmajor'])
        else:
            # 旧挙動: 実フレームの姿勢を使う
            T_wv_anchor = mat4_list_to_np(frame.pose.transform)
        Twc = world_to_cam_from_T_wv(T_wv_anchor, calib)
        ...
        meta = {
            ...
            "anchor_pose_T_wv": T_wv_anchor.reshape(-1).tolist(),
